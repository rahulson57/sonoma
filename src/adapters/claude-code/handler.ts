 ...
 *
 * Every step below is a module-level function taking the invocation it serves, so each one can be read on its own.
 */
...
/** HookHandlerOptions with every default applied (all but env, which only handleHook reads). */
interface HandlerDeps {
  readonly engine: AdapterEngine;
  readonly ledger: AdapterLedgerReader;
  readonly readStatus: ReadWorkspaceStatus;
  readonly retry: LockRetryPolicy;
  readonly sleep: (ms: number) => Promise<void>;
  readonly stderr: (line: string) => void;
}

/** One handleHook invocation: the handler's dependencies, the run it is bound to, and how much of its lock-wait budget it has slept. */
...
function resolveDeps(options: HookHandlerOptions): HandlerDeps {
  ...
  return {
    ...
    readStatus: options.readWorkspaceStatus ?? gitWorkspaceStatus,
    retry,
...
async function append(call: Invocation, observation: Observation): Promise<LedgerEvent> {
  const [event] = await withLockRetry(call, () => call.deps.engine.record([{ run_id: call.runId, ...observation }]));
...
async function appendError(call: Invocation, hook: string | null, stage: string, err: unknown): Promise<EventId | null> {
  try {
    ...
    try {
      call.deps.stderr(`ckpt hook ${hook ?? '(unparsed)'}: observation not recorded (${errorKind(appendErr)})\n`);
    } catch {
...
async function statusOf(call: Invocation): Promise<{ status: WorkspaceStatus | null; error: unknown }> {
  try {
    return { status: await call.deps.readStatus(await call.deps.engine.workspaceDir(call.runId)), error: undefined };
  } catch (err) {
...
async function requestedStatus(call: Invocation, toolUseId: string | null, beforeSeq: number): Promise<string | null> {
  if (toolUseId === null) return null;
  const { ledger } = call.deps;
  const floor = Math.max(1, beforeSeq - MAX_REQUEST_SCAN);
  for (let to = beforeSeq - 1; to >= floor; ) {
    const from = Math.max(floor, to - SCAN_PAGE + 1);
    const events = await ledger.getEvents(call.runId, { fromSeq: from, toSeq: to });
...
async function sessionStart(call: Invocation, hook: Extract<ParsedHook, { kind: 'SessionStart' }>): Promise<EventId> {
  ...
  const head = await call.deps.ledger.getEvents(call.runId, { fromSeq: 1, toSeq: 2 });
  const first = head[0];
...
async function preToolUse(call: Invocation, hook: Extract<ParsedHook, { kind: 'PreToolUse' }>): Promise<EventId> {
  ...
  const observed = WORKSPACE_TOOLS.has(hook.tool) ? await statusOf(call) : { status: null, error: undefined };
...
async function postToolUse(call: Invocation, hook: Extract<ParsedHook, { kind: 'PostToolUse' }>): Promise<EventId> {
  ...
  try {
    ...
    const before = await requestedStatus(call, hook.toolUseId, completed.seq);
...
async function boundary(call: Invocation, hook: Extract<ParsedHook, { kind: 'Stop' | 'SessionEnd' }>): Promise<EventId> {
  ...
  try {
    ...
    await withLockRetry(call, () => call.deps.engine.checkpoint(call.runId));
  } catch (err) {
...
/** One invocation, end to end: bind to the run, parse, dispatch; any failure becomes adapter.error. Never rejects. */
async function handleInvocation(
  ...
  const call: Invocation = { deps, runId, waitedMs: 0 };
...
export function createHookHandler(options: HookHandlerOptions): HookHandler {
  ...
  return {
    handleHook(payload: unknown): Promise<EventId | null> {
      return handleInvocation(deps, env, payload);
    },
  };
}
